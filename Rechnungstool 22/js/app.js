...
// Inject editable sequence controls into firm form
(function(){
  const afterEl=$('#cred_city')||$('#cred_country');
  if(!afterEl) return;
  if($('#cred_seq_counter')) return; // already injected
  afterEl.insertAdjacentHTML('afterend', `
    <div class="row" id="firm_seq_row" style="margin-top:8px">
      <div class="inline" style="gap:8px;align-items:center">
        <label style="min-width:140px">Rechnungszähler</label>
  ...
  `);
  const y=new Date().getFullYear();
  $('#cred_seq_year').value=String(y);
  $('#cred_seq_counter').value='1';
})();

// Inject QR Einzahlungschein (SVG) upload controls into firm form
(function(){
  const after=document.getElementById('firm_seq_row')||document.getElementById('cred_country');
  if(!after) return;
  if(document.getElementById('cred_qr_template')) return; // already injected
  after.insertAdjacentHTML('afterend', `
    <div class="row" id="firm_qr_row" style="margin-top:8px">
      <div class="inline" style="gap:8px;align-items:center">
        <label style="min-width:140px">QR Einzahlungschein (SVG)</label>
...
function refreshDropdowns(){
  ...
  if(selPC){
    const cur=selPC.value;
    selPC.innerHTML='<option value="">– Kunde wählen –</option>';
    customers.forEach((c,i)=>{
      const name=c.isCompany?c.companyName:`${c.fname} ${c.lname}`;
      selPC.append(new Option(name,i));
    });
    if(cur) selPC.value=cur;
  }
...
$('#add_item')?.addEventListener('click',()=>{const i=$('#sel_product').value;const q=Number($('#qty').value||1);if(i===''||!(q>0)){alert('Produkt & Menge');return;}addRow(Number(i),q);});$('#vat')?.addEventListener('input',recalc);
...
$('#passenger_customer_select')?.addEventListener('change',e=>{const val=e.target.value; if(val===''){return;} const c=customers[Number(val)]; if(!c)return; if(c.isCompany){ // Firmenkunde → leer lassen? Oder Kontaktperson nutzen
  $('#passenger_fname').value='';
  $('#passenger_lname').value=c.companyName;
}else{
  $('#passenger_fname').value=c.fname||'';
  $('#passenger_lname').value=c.lname||'';
}}
);
...
$('#save_invoice')?.addEventListener('click',()=>{const status=$('#status');status.textContent='';const firm=getSelectedFirm();if(!firm){alert('Keine Firma');return;}const ci=Number($('#sel_customer').value);const cust=customers[ci];if(!cust){alert('Kunde fehlt');return;}const title=($('#inv_title')?.value||'').trim();if(!title){alert('Titel fehlt');$('#inv_title').focus();return;}const rows=Array.from($('#items tbody').querySelectorAll('tr[data-row]'));const items=rows.map((tr,i)=>({pos:i+1,name:tr.children[1].textContent.trim(),qty:Number(tr.querySelector('.cell-qty input').value),price:Number(tr.querySelector('.cell-price input').value),total:Number(tr.dataset.total)}));if(!items.length){alert('Position hinzufügen');return;}if(editingInvoiceId){const inv=invoices.find(i=>i.id===editingInvoiceId);if(inv){inv.no=$('#inv_no').value||inv.no;inv.title=title;inv.firmId=firm.id;inv.customerIndex=ci;inv.vat=Number($('#vat').value||0);inv.items=items;inv.total=$('#grand').textContent;inv.flights=currentFlights.map(f=>({...f}));inv.passengers=currentPassengers.map(p=>({...p}));}editingInvoiceId=null;status.textContent='✅ Aktualisiert';}else{const auto=$('#inv_auto_no')?.checked; if(auto){ const newNo=consumeInvoiceNoForFirm(firm); $('#inv_no').value=newNo; } const inv={id:Date.now(),no:$('#inv_no').value||String(Date.now()),title,firmId:firm.id,customerIndex:ci,vat:Number($('#vat').value||0),items,total:$('#grand').textContent,created:new Date().toISOString(),flights:currentFlights.map(f=>({...f})),passengers:currentPassengers.map(p=>({...p})),issued:false};invoices.push(inv);status.textContent='💾 Gespeichert';$('#inv_title').value='';}
...
function handleInvoiceButton(b){
  ...
  if(act==='edit'){
    ...
    $('#inv_title').value=inv.title||'';$('#inv_no').value=inv.no||'';$('#firm_select').value=String(inv.firmId);$('#sel_customer').value=String(inv.customerIndex);$('#vat').value=inv.vat||0;
  ...
  if(act==='view'){
    ...
    const itemsRows=inv.items.map(it=>`<tr><td>${it.pos}</td><td>${esc(it.name)}</td><td class='r'>${it.qty}</td><td class='r'>${it.price.toFixed(2)}</td><td class='r'>${it.total.toFixed(2)}</td></tr>`).join('');
